...
class Object():
    ...
    def intersectLineWithPolygon(self,p0,p1,tol=1e-7):
        ...
        T  = vec.dot(N,Ptriangles[:,0]-p0) / den 
                
        # This equation is basically p = p0 + t*(p1-p0)
        P = p0 + vec.listTimesVec(T,p1-p0)
    ...
    def vtkActor(self):
        ...
        if self.normals is not None:
            ...
            nrm.SetNumberOfTuples(len(self.points))
            for n in range(len(self.points)):
                nrm.SetTuple(n,self.normals[n].tolist())
            me.GetPointData().SetNormals(nrm)
        ...
        dataActor.GetProperty().SetOpacity(self.opacity)
...
class Assembly(Object):  
    ...
    def intersectLineWithPolygon(self,p0,p1,tol=1e-7):
        result = Object.intersectLineWithPolygon(self,p0,p1,tol)
        if result is None:
            result = {}
        for o in self.objectList:
            temp = o.intersectLineWithPolygon(self,p0,p1,tol)
            if temp is not None:
                result = dict(result.items() + temp.items())
        return result
    
    def getBounds(self):
        result = Object.getBounds(self)
        for o in self.objectList:
            temp = o.getBounds()
            result = (result * (temp > result) + temp * (temp < result)) * \
                      np.array([1,0,1,0,1,0]) + \
                     (result * (temp < result) + temp * (temp > result)) * \
                      np.array([0,1,0,1,0,1])
                      
        xmin = [result[0],result[2],result[4]]
        xmax = [result[1],result[3],result[5]]
        self.bounds = [xmin,xmax]             
        return result
                      
    def translate(self,v):
        Object.translate(self,v)
        for o in self.objectList:
            o.translate(v)
    ...
    def rotateAroundAxis(self,angle,axis,pivotPoint=None):
        ...
        Object.rotateAroundAxis(self,angle,axis,pivotPoint)
        for o in self.objectList:
            o.rotateAroundAxis(angle,axis,pivotPoint)
            
    def alignTo(self,direction,normal,lateral=None):
        Object.alignTo(self,direction,normal,lateral)
        for o in self.objectList:
            o.alignTo(direction,normal,lateral)
            
    def destroyData(self):
        Object.destroyData(self)
        for o in self.objectList:
            o.destroyData()
            
    def vtkActor(self):
        actorlist = Object.vtkActor(self)
        for o in self.objectList:
...
if __name__=="__main__":
    ...
    nm = 30000
    for n in range(nm):
        mesh.intersectLineWithPolygon(np.array([0.5,0.5,-1]),
                                         np.array([0.5,0.5,2]))
        mesh.intersectLineWithPolygon(np.array([1.5,1.5,-1]),
                                         np.array([1.5,1.5,2]))  
        mesh.intersectLineWithPolygon(np.array([0.999999,2,1]),
                                         np.array([1,-1,1])) 
        mesh.intersectLineWithPolygon(np.array([-1,-1e-6,-1e-6]),
                                         np.array([2,0,0]))
    tic.toctask(4*nm*12)                                
